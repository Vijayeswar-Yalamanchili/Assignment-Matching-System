...
const AssignmentSubmitModal = ({ assignmentId }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      let res = await AxiosService.post(`${ApiRoutes.SUBMITASSIGNMENT.path}/${userId}`,form,{ headers: { Authorization: ` ${getLoginToken}`}})
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 backdrop-blur-md">
      <div className="bg-white p-6 rounded-lg shadow-lg w-140 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <input type="text" name="githubRepo" placeholder="Github Repo Link" required className="w-full p-2 border rounded" value={form.gitHub_repo_link} onChange={(e) =>setForm((prev) => ({ ...prev, gitHub_repo_link: e.target.value.trim() }))}/>
          <label>Demo Link</label>
          <input type="text" name="demolink" placeholder="Demo Link" required className="w-full p-2 border rounded" value={form.demo_link} onChange={(e) =>setForm((prev) => ({ ...prev, demo_link: e.target.value.trim() }))}/>
